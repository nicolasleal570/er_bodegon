...
export class EmpleadoForm extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            formControls: {
                ...
                cargo: {
                    placeholder: 'Inserta el cargo',
                    type: 'text',
                    value: ''
                },
                sueldo: {
                    placeholder: 'Inserta el sueldo',
                    type: 'text',
                    value: ''
                },
            },
    ...
    createNewProduct(empleado) {
        let usuario = {
            'name': empleado.name,
            'phone': Number(empleado.phone),
            'cedula': Number(empleado.cedula),
            'birthday': empleado.birthday,
            'ubication': empleado.ubication,
            'is_available': true
        }
        ...
        axios.post('http://localhost:8000/api/usuarios/', { usuario }).then(res => {
            newEmpleado.usuario_id = res.data.usuario.id;
            axios.post('http://localhost:8000/api/empleados/', { 'empleado': newEmpleado }).then(respon => {
                console.log(res.data, respon.data);
                this.setState({ redirect: "/empleados" });
            });
        }).catch(err => {
            const errors = {
                exist: true,
    ...
    updateProduct(cliente) {
        ...
        axios.put(`http://localhost:8000/api/clientes/${this.state.clientId}`, { 'cliente': newEmpleado }).then(res => {
                        ...
                        <div className="card-body">
                            <form onSubmit={this.handleSubmit} className="form-group">
                                ...
                                <TextInputField name="ubication"
                                    ...
                                    onChange={this.handleChange} />
                                <TextInputField name="cargo"
                                    placeholder={this.state.formControls.cargo.placeholder}
                                    value={this.state.formControls.cargo.value}
                                    onChange={this.handleChange} />
                                <TextInputField name="sueldo"
                                    placeholder={this.state.formControls.sueldo.placeholder}
                                    value={this.state.formControls.sueldo.value}
                                    onChange={this.handleChange} />

                                {
                                    this.state.handleErrors.exist ? <ErrorHandlerAler errors={this.state.handleErrors.errors} keys={['name', 'phone', 'cedula', 'birthday', 'ubication', 'cargo', 'sueldo']} /> : ''
                                }
                                <input type="submit" value="Guardar Empleado" className="btn btn-primary" />
                            </form>
                        </div>
                    </div>
                </div>
